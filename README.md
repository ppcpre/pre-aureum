...
- ✅ **หน้าใหม่ "RSI & แนวรับแนวต้าน" (2026-09-09)**: ตามที่ขอ ("อยากได้อีกกราฟ ตีเส้นกรอบมาให้แบบตัวอย่างนี้เลย แล้วบอกจุดตัด" + ภาพตัวอย่างเส้นแนวโน้มทแยง/RSI divergence 2 ภาพ) — สร้างเป็นหน้าใหม่แยกต่างหาก (`/trend-analysis`, เมนู "RSI & แนวรับแนวต้าน" ใต้ ทอง) **ไม่แตะ Dashboard เดิม** ตามที่ขอ เลือก timeframe ได้ (M15/H1/H4/D1/W1)
  ...
  - **✅ เพิ่ม EMA50/EMA200 ทับกราฟราคา (2026-09-11)**: ตามที่ขอ ("ทำ EMA บนกราฟต่อเลย") — ออกแบบ mockup ก่อนแล้วค่อยทำจริงตามที่คุยกันไว้ (เส้นบางสีอำพัน=EMA50, สีฟ้าอมเทา=EMA200 ไม่ชนโทนเดิมของแอป) เพิ่ม `calculateEMASeries()` ใน `sr-engine.ts` (`calculateEMA()` เดิมกลายเป็น wrapper บาง เหมือนที่ทำกับ RSI) — วาดเป็นเส้นทับกราฟราคาบนหน้านี้เท่านั้น
    - **Golden Cross / Death Cross**: จุดตัดที่ EMA50 ตัดผ่าน EMA200 (`findEMACrossings` ใน `trend-analysis.ts`) ขึ้น marker ลูกศรบนกราฟ (เขียวขึ้น=Golden Cross ขาขึ้น, แดงลง=Death Cross ขาลง) พร้อมรวมเข้ากับรายการ "จุดตัด" การ์ดเดียวกับ RSI=50 (เรียงตามวันที่จริง ไม่แยกการ์ด)
    - **ข้อจำกัดที่บอกตรงๆ**: EMA200 ต้องการ 200 แท่ง จากที่ backfill ไว้ 210 แท่ง/timeframe เหลือแค่ ~11 จุดข้อมูลสำหรับ EMA200 (เส้นสั้น ใกล้ขอบขวาของกราฟเท่านั้น) — golden/death cross ที่เกิดขึ้นก่อนหน้าช่วง 11 แท่งนี้จะไม่ถูกตรวจจับ ถ้าอยากได้ประวัติยาวขึ้นต้องเพิ่ม `GOLD_CANDLE_COUNT` ใน `gold-refresh.ts` (แลกกับ backfill ที่ใหญ่ขึ้น)
    - ทดสอบยืนยันบน production แล้วทั้ง H4/W1: เส้น EMA ทั้งสองขึ้นถูกสี ตำแหน่งตรงกับราคาจริง ไม่มี console error

...
- ✅ **หน้าใหม่ "RSI & แนวรับแนวต้าน" (2026-09-09)**: ตามที่ขอ ("อยากได้อีกกราฟ ตีเส้นกรอบมาให้แบบตัวอย่างนี้เลย แล้วบอกจุดตัด" + ภาพตัวอย่างเส้นแนวโน้มทแยง/RSI divergence 2 ภาพ) — สร้างเป็นหน้าใหม่แยกต่างหาก (`/trend-analysis`, เมนู "RSI & แนวรับแนวต้าน" ใต้ ทอง) **ไม่แตะ Dashboard เดิม** ตามที่ขอ เลือก timeframe ได้ (M15/H1/H4/D1/W1)
  ...
  - **✅ เพิ่ม MACD (2026-09-25)**: ตามที่ขอ ("เพิ่มมาด้วย" ต่อจากคำถาม "ดูยังไงนะ ทั้ง MACD กับ EMA") — เพิ่ม panel ที่ 3 ใต้กราฟ RSI บนหน้าเดียวกัน (price → RSI → MACD ซิงค์ zoom/pan กันทั้ง 3 กราฟแล้ว ขยายจาก sync 2 ทางเดิมเป็นแบบ N ทาง)
    - `calculateMACDSeries()` ใน `sr-engine.ts` (12/26/9 มาตรฐาน) — ดึง EMA primitive เดิมออกมาเป็น `emaOfValues()` ใช้ร่วมกันได้กับค่าซีรีส์ไหนก็ได้ ไม่ใช่แค่ราคาปิด เพราะเส้น Signal ของ MACD คือ EMA ของ**เส้น MACD เอง** ไม่ใช่ราคา
    - โชว์: แท่ง histogram (เขียว/แดงตามเครื่องหมาย) + เส้น MACD (ทอง) + เส้น Signal (ฟ้า เฉดเดียวกับ EMA200) + เส้นอ้างอิง 0 — จุดตัด MACD/Signal (`findMACDCrossings`) ขึ้นจุดวงกลมบนกราฟ และรวมเข้ารายการ "จุดตัด" การ์ดเดียวกับ RSI=50/EMA (เปลี่ยนหัวข้อเป็น "RSI=50 / EMA / MACD")
    - MACD ต้องการแค่ 34 แท่ง (slowPeriod 26 + signalPeriod 9 − 1) ต่างจาก EMA200 ที่ต้องการ 200 แท่ง — กับ backfill 210 แท่งปัจจุบัน MACD เลยมีข้อมูลเกือบเต็มกราฟ (~177 จุดจาก 210 แท่ง) ไม่ใช่เส้นสั้นๆ แบบ EMA200
    - ทดสอบยืนยันบน production แล้ว: MACD panel เรนเดอร์ถูกต้อง มี crossing จริงหลายจุด (12 จุดใน H4), รายการ "จุดตัด" รวม RSI+EMA+MACD ถูกต้อง (35 รายการ, 12 เป็น MACD), มือถือไม่ overflow, ไม่มี console error
